...
Imports System.Linq
Imports System.IO
Imports Helpdesk_Application.ClsConn
...
Public Class utama
    ...
    Dim AppTicket, AppAll As String
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        Else
            ...
            If AppTicket = "Yes" Then
                ...
                div_button_ticket.Visible = True
                div_button_sosmed.Visible = False
            Else
                div_ticket.Visible = False
                div_button_ticket.Visible = False
                div_button_sosmed.Visible = True
            End If

            If Request.QueryString("channel") = "facebook" Then
                function_Scustomer_Facebook(Request.QueryString("id"), Request.QueryString("account"))
                sql_Source_Master()
                Btn_Assign.Visible = False
            ElseIf Request.QueryString("channel") = "twitter" Then
                function_Scustomer_Twitter(Request.QueryString("id"), Request.QueryString("account"))
                sql_Source_Master()
                Btn_Assign.Visible = False
            ElseIf Request.QueryString("channel") = "email" Then
                function_Scustomer_Email(Request.QueryString("id"), Request.QueryString("account"))
                sql_Source_Master()
                Btn_Assign.Visible = False
            End If
        End If
    ...
    Function function_Scustomer_Email(ByVal id As String, ByVal account As String)
        sql = "select * from mcustomer where Email='" & account & "'"
        sqldr = connection.ExecuteReader(sql)
        Try
            If sqldr.HasRows() Then
                sqldr.Read()
                If sqldr("NamaPerusahaan").ToString = "" Then
                    lbl_nama_customer.Text = account
                Else
        ...
        sql = "select * from IVC_EMAIL_IN_TM where IVC_ID='" & id & "'"
        sqldr = connection.ExecuteReader(sql)
        If sqldr.HasRows() Then
            sqldr.Read()
            lbl_message.Text = sqldr("ESUBJECT").ToString
            lbl_date.Text = sqldr("Email_Date")
        Else

        End If
        sqldr.Close()
        lbl_pic_customer.Text = Request.QueryString("account")
        lbl_customer.Text = lbl_nama_customer.Text
        cmb_source_type.Value = Request.QueryString("channel")
        cmb_source_type.Enabled = False
        txt_customer_name.Text = Request.QueryString("account")
        txt_customer_name.Enabled = False
        txt_message.Visible = False
        lbl_red.Visible = False
        hprlink.NavigateUrl = "inbox_email.aspx"
        lbl_link.Text = "Inbox Email"
    End Function

    Function function_Scustomer_Facebook(ByVal id As String, ByVal account As String)
        sql = "select * from mcustomer where Facebook='" & account & "'"
        sqldr = connection.ExecuteReader(sql)
        Try
            If sqldr.HasRows() Then
                sqldr.Read()
        ...
        Catch ex As Exception
            Response.Write(ex.Message)
        End Try
        sql = "select * from fb_tGet_Header where facebook_ID='" & id & "'"
        sqldr = Sosmed.ExecuteReader(sql)
        If sqldr.HasRows() Then
            sqldr.Read()
            lbl_message.Text = sqldr("facebook_Message").ToString
            lbl_date.Text = sqldr("ddate")
        Else
    ...
    Function function_Scustomer_Twitter(ByVal id As String, ByVal account As String)
        sql = "select * from mcustomer where Twitter='" & account & "'"
        sqldr = connection.ExecuteReader(sql)
        Try
            If sqldr.HasRows() Then
                sqldr.Read()
        ...
        Catch ex As Exception
            Response.Write(ex.Message)
        End Try
        sql = "select * from tw_tMention where id_Twitter='" & id & "'"
        sqldr = Sosmed.ExecuteReader(sql)
        If sqldr.HasRows() Then
            sqldr.Read()
            lbl_message.Text = sqldr("tText").ToString
            lbl_date.Text = sqldr("created_at")
        Else
    ...
    Private Sub btn_update_customer_Click(sender As Object, e As EventArgs) Handles btn_update_customer.Click
    ...
    Private Sub Btn_Simpan_Click(sender As Object, e As EventArgs) Handles Btn_Simpan.Click
        ...
        If cmb_status.Value = "Closed" Then
            ...
            If hd_nik.Value <> "" Then
                comm.Parameters.Add("@nik", Data.SqlDbType.VarChar).Value = hd_nik.Value
                ParamCustomer = hd_nik.Value
            Else
                comm.Parameters.Add("@nik", Data.SqlDbType.VarChar).Value = hd_nik.Value
                ParamCustomer = hd_nik.Value
            End If
            ...
            comm.Parameters.Add("@dateAgentResponse", Data.SqlDbType.VarChar).Value = "2016-08-31 10:11:56.000"
            ...
            comm.Parameters.Add("@atasjamkerja", Data.SqlDbType.VarChar).Value = DateNow
    ...
    Private Sub btn_dispatch_unit_Click(sender As Object, e As EventArgs) Handles btn_dispatch_unit.Click
        log.writedata("--Start--")
        log.writedata("UserName/Date : " & Session("UserName") & "/" & DateTime.Now)
        log.writedata("Ticket Number: " & lbl_ticket_number.Text)
    ...
    Private Sub btn_simpan_sosmed_Click(sender As Object, e As EventArgs) Handles btn_simpan_sosmed.Click
        Dim count As Integer
        conn = New SqlConnection(connectionString)
        comm = New SqlCommand()
        comm.Connection = conn
        ...
        comm.Parameters.Add("@sourcetype", Data.SqlDbType.VarChar).Value = cmb_source_type.Value
        comm.Parameters.Add("@grouptype", Data.SqlDbType.VarChar).Value = cmb_group_type.Value
        comm.Parameters.Add("@channel", Data.SqlDbType.VarChar).Value = Request.QueryString("channel")
        comm.Parameters.Add("@descriptionmessage", Data.SqlDbType.VarChar).Value = lbl_message.Text
        comm.Parameters.Add("@solution", Data.SqlDbType.VarChar).Value = html_solution.Html
        comm.Parameters.Add("@attch", Data.SqlDbType.VarChar).Value = "test"
        comm.Parameters.Add("@usercreate", Data.SqlDbType.VarChar).Value = Session("username")
        Try
            conn.Open()
            reader = comm.ExecuteReader()
