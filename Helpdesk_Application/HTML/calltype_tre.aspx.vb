...
Partial Class calltype_tre
    ...
    Protected Sub ASPxGridView1_RowInserting(ByVal sender As Object, ByVal e As DevExpress.Web.Data.ASPxDataInsertingEventArgs) Handles ASPxGridView1.RowInserting
        ...
        SourceJenisTransaksi = "Select COUNT (SubName) as CekTransaksi from mSubCategoryLv3 where CategoryID='" & cekJenisTransaksi & "' and SubCategory1ID='" & cekUnitKerja & "' and SubCategory2ID='" & cekNameSubject & "' and SubName='" & cekSubjectTable & "'"
        ...
        Dim cekIDkamus As String
        cekIDkamus = "Select COUNT (SubName) as CekTransaksi from mSubCategoryLv3 where IDKamus='" & e.NewValues("IDKamus").ToString & "'"
        Comm = New SqlCommand(cekIDkamus, Connection)
        Connection.Open()
        Dr = Comm.ExecuteReader()
        Dr.Read()
        cekIDkamus = Dr("CekTransaksi").ToString
        Dr.Close()
        Connection.Close()
        ...
        Dim querySelectData As String = "select SUBSTRING(SubCategory3ID,5,5) as nourutAkhir from mSubCategoryLv3 order by ID desc"
        ...
        If nourutAkhir = "" Then
            querySelectData = "select  AngkaTerakhir = Right(10001 + COUNT(ID) + 0, 5)  from mSubCategoryLv3"
        Else
            querySelectData = "select  AngkaTerakhir = Right(100" & nourutAkhir & " + 1 + 0, 5)  from mSubCategoryLv3"
        End If
        ...
        GenerateNoID = "CT3-" & angkaTerakhir & ""
        DsCategory.InsertCommand = "insert into mSubCategoryLv3 (CustomerID,IDKamus,CategoryID,SubCategory1ID,SubCategory2ID,SubCategory3ID,SubName,SLA,Response_Agent,Priority,Severity,NA,Status_Customer,Version,License) values (@NamaPerusahaan,@IDKamus,@JenisTransaksi,@UnitKerja,@NameSubject,'" & GenerateNoID & "',@SubjectTable,@SLA,@Response_Agent,@Priority,@Severity,@NA,@Status_Customer,@Version,@License)"
    End Sub
